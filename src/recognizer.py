...
class SwinRecognizer:
    """Swin-Tiny character classifier for v11/v12 inference."""

    def __init__(self, model_path, mapping_dir=None, device="cpu"):
        self.model_path = str(model_path)
        if device in ("cuda", "cuda:0") and not torch.cuda.is_available():
            device = "cpu"
        self.device = device
        print(f"Loading Swin recognizer: {model_path} on {self.device}")
        import timm, torch.nn as nn
        backbone = timm.create_model('swin_tiny_patch4_window7_224', pretrained=False, num_classes=0)
        class SwinClassifier(nn.Module):
            def __init__(self):
                super().__init__()
                self.backbone = backbone; self.pool = nn.AdaptiveAvgPool2d((1,1)); self.head = nn.Linear(768, 3483)
            def forward(self, x):
                feats = self.backbone.forward_features(x); feats = feats.permute(0,3,1,2)
                return self.head(self.pool(feats).flatten(1))
        ...
        self.num_classes = 3483; self.input_size = 224; self._resize_size = 256
        print(f"  Loaded: epoch={ckpt.get('epoch', '?')}")
        self.idx_to_class = {}; self.class_to_char = {}
        self._load_mappings(mapping_dir)
        from torchvision import transforms
        self.transform = transforms.Compose([
    ...
    def _load_mappings(self, mapping_dir):
        ...
        if idx_path.exists():
            with open(idx_path, encoding='utf-8') as f:
                raw = json.load(f)
                for k, v in raw.items(): self.idx_to_class[str(k)] = v
        if not self.idx_to_class: print("  WARNING: No idx_to_class mapping found!"); return
        sample_val = next(iter(self.idx_to_class.values()))
        is_direct_char = len(sample_val) == 1 and ord(sample_val) > 127
        if is_direct_char: self.class_to_char = dict(self.idx_to_class)
    ...
    def predict(self, image, topk=5):
        ...
        if isinstance(image, str): image = Image.open(image).convert('RGB')
            ...
            from torchvision.transforms.functional import to_pil_image; image = to_pil_image(image)
        tensor = self.transform(image).unsqueeze(0).to(self.device)
        logits = self.model(tensor); probs = F.softmax(logits, dim=1)
        topk_probs, topk_indices = torch.topk(probs, min(topk, self.num_classes), dim=1)
        results = []
        for i in range(len(topk_indices[0])):
            idx = str(topk_indices[0][i].item()); prob = topk_probs[0][i].item()
            cls_id = self.idx_to_class.get(idx, idx); chinese = self.class_to_char.get(idx, cls_id)
            results.append({"rank":i+1,"class_index":int(idx),"class_id":cls_id,"char":chinese,"confidence":round(prob,4)})
        return results

    @torch.no_grad()
    def predict_batch(self, images, topk=5):
        tensors = torch.stack([self.transform(img) for img in images]).to(self.device)
        logits = self.model(tensors); probs = F.softmax(logits, dim=1)
        topk_probs, topk_indices = torch.topk(probs, min(topk, self.num_classes), dim=1)
        batch_results = []
        for b in range(len(images)):
            results = []
            for i in range(len(topk_indices[b])):
                idx = str(topk_indices[b][i].item()); prob = topk_probs[b][i].item()
                cls_id = self.idx_to_class.get(idx, idx); chinese = self.class_to_char.get(idx, cls_id)
                results.append({"rank":i+1,"class_index":int(idx),"class_id":cls_id,"char":chinese,"confidence":round(prob,4)})
            batch_results.append(results)
        return batch_results
